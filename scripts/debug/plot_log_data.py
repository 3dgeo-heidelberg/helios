...
if not os.path.isdir(sys.argv[2]):
    if os.path.exists(sys.argv[2]):
        print('"{path}" exists but it is not a directory'.format(path=sys.argv[2]))
        sys.exit(2)
    else:
...
def title_from_index(index):
    ...
    if kdtNum == 1:
        ...
        kdtType = "FastSAH"
    if parallelNum == 0:
        parallelType = "Static scheduling with chunk size {chunkSize}".format(
            chunkSize=chunkSize
        )
        if chunkSize < 0:
            parallelType = "Dynamic scheduling with chunk size {chunkSize}".format(
                chunkSize=chunkSize
            )
    elif parallelNum == 1:
        parallelType = "Warehouse x{whFactor} with chunk size {chunkSize}".format(
            whFactor=whFactor, chunkSize=chunkSize
        )

    # Return title
    return (
        "Helios with {kdtType} KDTree of {sahNodes} nodes\n"
        "{parallelType} parallelization".format(
            kdtType=kdtType, sahNodes=sahNodes, parallelType=parallelType
        )
    )
...
def name_from_index(index):
    ...
    chunkSize = index[3]
    whFactor = index[4]
    return "KDT{kdt}_SAH{sah}_PS{ps}_CS_{cs}_WF{wf}".format(
        kdt=kdtNum, sah=sahNodes, ps=parallelNum, cs=chunkSize, wf=whFactor
    )
...
def find_summary_best_parallelization(summary_entry):
    ...
    if bestType == "Static":
        ...
        bestType = "Ware. {cs}x{wf}".format(
            cs=bestEntry["chunkSize"], wf=bestEntry["warehouseFactor"]
        )
...
def plot_summary(summary):
        ...
        plots.append(
            ax.plot(
                ...
                label="{kdtType} {paralleliz} time".format(
                    kdtType=kdtTypeStr, paralleliz=paralleliz
                ),
            )[0]
        )
        plots.append(
    ...
    # Export plot
    plt.savefig(
        fname="{outpath}".format(
            outpath="{outdir}summary_plot".format(
                outdir=outdir, plotname=name_from_index(KDT.index[0])
            )
        )
    )
    plt.close()
...
def configure_plot(ax, xlabel="Cores", ylabel="Seconds", axx=None, yxlabel="Speed-up"):
...
for DF in DFS:
    ...
    KDT = (
        DF.drop("SimulationCores", axis=1)
        .groupby(BASE_GROUP_LIST + ["KDTBuildingCores"])
        .mean()
    )
    SIM = (
        DF.drop("KDTBuildingCores", axis=1)
        .groupby(BASE_GROUP_LIST + ["SimulationCores"])
        .mean()
    )
    ...
    ax.plot(kdtCores, kdtTime, lw=2, ls="-", color="blue", label="KDT building time")
    if markOptimumTime:
        ax.scatter(
    ...
    axx = ax.twinx()
    axx.plot(kdtCores, kdtCores, color="black", lw=1, ls="--", label="Ideal speed-up")
    axx.plot(
        kdtCores, kdtSpeedup, lw=1, ls="-", color="black", label="KDT building speed-up"
    )
    configure_plot(ax, xlabel="KDT Building cores", axx=axx)

    # Subplot 2
    ax = plt.subplot(2, 2, 2)
    ax.plot(simCores, simTime, lw=2, ls="-", color="green", label="Simulation time")
    if markOptimumTime:
        ax.scatter(
    ...
    axx = ax.twinx()
    axx.plot(simCores, simCores, color="black", lw=1, ls="--", label="Ideal speed-up")
    axx.plot(
        simCores, simSpeedup, color="black", lw=1, ls="-", label="Simulation speed-up"
    )
    configure_plot(ax, xlabel="Simulation cores", axx=axx)

    # Subplot 3
    ax = plt.subplot(2, 2, 3)
    ax.plot(simCores, fullTime, lw=2, ls="-", color="red", label="Total time")
    if markOptimumTime:
        ax.scatter(
    ...
    axx = ax.twinx()
    axx.plot(simCores, simCores, color="black", lw=1, ls="--", label="Ideal speed-up")
    axx.plot(simCores, fullSpeedup, color="black", lw=1, ls="-", label="Total speed-up")
    ...
    plt.savefig(
        fname="{outpath}".format(
            outpath="{outdir}{plotname}".format(
                outdir=outdir, plotname=name_from_index(KDT.index[0])
            )
        )
    )
