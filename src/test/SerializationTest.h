...
SerializationTest::run()
{
  ...
  if (!validate(*(DetailedVoxel*)scene1.primitives[0],
                *(DetailedVoxel*)scene2->primitives[0]))
    return false;
  if (!validate(*(Triangle*)scene1.primitives[1],
                *(Triangle*)scene2->primitives[1]))
    return false;
  if (!validate(*(Voxel*)scene1.primitives[2], *(Voxel*)scene2->primitives[2]))
    return false;
  if (!validate(*(AABB*)scene1.primitives[3], *(AABB*)scene2->primitives[3]))
    return false;
  for (size_t i = 0; i < nRepeats; i++) {
    if (!validate(*(DetailedVoxel*)scene1.primitives[i + 4],
...
// *************** //
bool
SerializationTest::validate(DetailedVoxel& dv1, DetailedVoxel& dv2)
{
  glm::dvec3 dv1c = dv1.getCentroid();
  glm::dvec3 dv2c = dv2.getCentroid();
  if (dv1c.x != dv2c.x || dv1c.y != dv2c.y || dv1c.z != dv2c.z)
    return false;
  if (dv1.halfSize != dv2.halfSize)
    return false;
  for (size_t i = 0; i < dv1.getNumberOfIntValues(); i++) {
    if (dv1.getIntValue(i) != dv2.getIntValue(i))
      return false;
  }
  ...
  if (dv1.material != nullptr && dv2.material != nullptr) {
    for (size_t i = 0; i < 4; i++) {
      if (dv1.material->ka[i] != dv2.material->ka[i])
        return false;
    }
  }
  return true;
...
bool
SerializationTest::validate(Voxel& v1, Voxel& v2)
{
  glm::dvec3 v1c = v1.getCentroid();
  glm::dvec3 v2c = v2.getCentroid();
  if (v1c.x != v2c.x || v1c.y != v2c.y || v1c.z != v2c.z)
    return false;
  if (v1.halfSize != v2.halfSize)
    return false;
  return true;
}
bool
SerializationTest::validate(AABB& box1, AABB& box2)
{
...
bool
SerializationTest::validate(Triangle& t1, Triangle& t2)
{
