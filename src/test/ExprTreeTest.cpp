...
bool
validate(std::shared_ptr<IExprTreeNode<InputType, OutputType>> f,
         std::vector<InputType> const& t,
         std::vector<OutputType> const& y)
{
  std::vector<OutputType> ft;
  for (InputType const& ti : t)
    ft.push_back(f->eval(ti));
  if (ft.size() != y.size())
    return false;
...
bool
validatePowIPow(std::shared_ptr<UnivarExprTreeNode<NumericType>> f,
                ...
                size_t const expectedIPowCount)
{
  ...
  while (btdi.hasNext()) {
    ...
    if (!node->isOperator())
      continue;
    if (node->op == node->OP_POW)
      ++powCount;
    if (node->op == node->OP_IPOW)
      ++ipowCount;
  }
...
template<typename NumericType>
bool
validatePowIPow(std::shared_ptr<IExprTreeNode<NumericType, NumericType>> f,
                size_t const expectedPowCount,
                size_t const expectedIPowCount)
{
...
TEST_CASE("Univariate Expression Tree Tests")
{
  ...
  SECTION("1+(t-1)^2")
  {
    ...
    y = { 122,    18.1396, 14.8384, 5.41,   2.7956, 2,
          1.4356, 1.01,    3.9584,  5.5796, 82 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("exp(t)+3*cos(1+2*t)")
  {
    node = uetsf.makeShared("exp(t)+3*cos(1+2*t)");
    y = { 2.96615925, 1.65614048, -0.741224555, 1.41994435,
          3.55947658, 2.62090692, 1.07798733,   0.00928169656,
          18.1435115, 24.7328066, 22024.8226 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("ln(t/2)*sin(t)")
  {
    ...
    y = { NAN,         NAN,          NAN,          NAN,         NAN,
          NAN,         -0.590924735, -0.532796735, 0.125827096, 0.000718406901,
          -0.875568201 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("acos(1/3)+t/pi")
  {
    ...
    y = { -1.95213944, 0.23146637, 0.36515653, 0.88081854,
          1.12273406,  1.23095942, 1.33918478, 1.58110029,
          2.09676231,  2.23045246, 4.41405828 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("5*t")
  {
  ...
  SECTION("4*t^3 + 3*t^2 + 2*t - 1")
  {
    ...
    y = { -3721,    -101.537776, -64.739392, -4.894,     -1.490416, -1,
          0.184016, 10.154,      107.129792, 158.695376, 4319 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("3600 - t^(t+1)")
  {
    node = uetsf.makeShared("3600 - t^(t+1)");
    y = { 3600,      NAN,        NAN,       NAN,        NAN,           3600,
          3599.7644, 3598.77841, 3558.6385, 3485.89919, -99999996400.0 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("cos(t)")
  {
    node = uetsf.makeShared("cos(t)");
    y = { -0.83907153, -0.99999873, -0.91243836, 0.45359612,  0.94275467, 1,
          0.94275467,  0.45359612,  -0.91243836, -0.99999873, -0.83907153 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("sin(t^2 + 3)")
  {
    node = uetsf.makeShared("sin(t^2 + 3)");
    y = { 0.62298863,  0.28904527, -0.82692784, -0.87643472,
          0.02598973,  0.14112001, 0.02598973,  -0.87643472,
          -0.82692784, 0.28904527, 0.62298863 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("3.1 + 5 * 7 + 4.4")
  {
  ...
  SECTION("((t-1)*((1+t) / 2))/(t+0.01)^2")
  {
    ...
    y = { 0.495991487, 0.452163439, 0.435614983, 0.0883763993,
          -4.06060606, -5000,       -3.60979592, 0.0852203555,
          0.429255726, 0.446439909, 0.494011483 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("(pi/3)^(abs(t)^(e-1))")
  {
    node = uetsf.makeShared("(pi/3)^(abs(t)^(e-1))");
    y = { 11.14208743, 1.39014251, 1.29353943, 1.05582653, 1.00725079, 1,
          1.00725079,  1.05582653, 1.29353943, 1.39014251, 11.14208743 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("atan2(t, 1) + pi")
  {
    node = uetsf.makeShared("atan2(t, 1) +pi");
    y = { 1.67046498, 1.87911199, 1.92310505, 2.30861139,
          2.81385415, 3.14159265, 3.46933116, 3.97457392,
          4.36008026, 4.40407332, 4.61272033 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("t-(-t) == 2*t")
  {
  ...
  SECTION("1+atan2(3*t-1, 1)")
  {
    ...
    y = { -0.53854944, -0.47512005, -0.46205665, -0.34229969,
          -0.11111695, 0.21460184,  1.01999733,  2.16066899,
          2.43202915,  2.4525852,   2.53632723 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("1+atan2(3*t-1, (t-1)*2)")
  {
    node = uetsf.makeShared("1+atan2(3*t-1, (t-1)*2)");
    y = { -1.18798772, -1.24225167, -1.25294846, -1.34443033,
          -1.49570776, -1.67794504, 4.1264423,   2.48405799,
          2.12291318,  2.10052003,  2.01530259 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("t*atan2(-1, -t)")
  {
    node = uetsf.makeShared("t*atan2(-1, -t)");
    y = { 0.99668652,  0.96811118,  0.95827973,  0.81159657,  0.42263966,  0,
          -0.64550184, -2.64415535, -7.58685229, -8.89648975, -30.41924001 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("t+(-(t-1)) == t+(-(-1+t))")
  {
  ...
  SECTION("-(t-1)*(t^2-2*t)/10^3")
  {
    ...
    y = { 1.32,         0.066817944, 0.047758848,  0.007161,     0.001066104, 0,
          -0.000372504, 0.000099,    -0.003368448, -0.007660344, -0.72 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("-(-1)*(t^2-2*t)/10^3")
  {
    node = uetsf.makeShared("-(-1)*(t^2-2*t)/10^3");
    y = { 0.12,       0.0161396, 0.0128384, 0.00341,   0.0007956, 0,
          -0.0005644, -0.00099,  0.0019584, 0.0035796, 0.08 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("(1000+((1-(1+sin(1/2+3*pi/3*t))*(2+t))*abs(t)^(2-1/2)))^(3/2)")
  {
    ...
    y = { 52714.18945957, 32171.06013344, 31838.59857288, 31637.35056204,
  ...
  SECTION("cos(3*t+sin(1+t)-5*(10+tan(t))^(2*sqrt(abs(t))))")
  {
    ...
    y = { 0.91282919,  0.97082196,  0.90930816, -0.9976885,
          -0.01534952, -0.52597404, 0.49827694, -0.99743291,
          0.3236059,   0.7942002,   -0.98567806 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("cos(3*t+atan2(t,1)-5*atan2(1/1+1-1,t/t*t)^(2*sqrt(abs(t))))")
  {
    ...
    y = { -0.45058035, -0.83062706, 0.94696144, -0.49875059,
          0.79075789,  0.28366219,  0.37464262, 0.0799824,
          -0.9787685,  -0.38051728, 0.99847693 };
    REQUIRE(validate(node, t, y));
  }
  SECTION("(t+10)^2-(t+10)^3")
  {
    node = uetsf.makeShared("(t+10)^2-(t+10)^3");
    y = { 0,    -275.769256, -332.829952, -625.759,     -808.113096,
          -900, -998.591704, -1244.421,   -1896.277248, -2096.087544,
          -7600 };
    REQUIRE(validate(node, t, y));
  ...
  SECTION("1000*((t+10)^2.0000001-(t+10)^2)")
  {
    ...
    y = { 0,         0.0090623,  0.01052088, 0.01731571, 0.02116392, 0.02302585,
          0.0249757, 0.02965597, 0.04114818, 0.04447127, 0.11982931 };
    REQUIRE(validate(node, t, y));
...
TEST_CASE("Register Univariate Expression Tree Tests")
{
  ...
  SECTION("abs(-t) - abs(-1*t) + abs(t) - abs(-2*t+(t))")
  {
    ...
    ER1 = 2;
    node =
      ruetsf.makeShared("abs(-t) - abs(-ER0*t) + abs(t) - abs(-ER1*t+(t))");
    y = std::vector<double>(t.size(), 0);
    REQUIRE(validate(node, t, y));
  }
  SECTION("1 + atan2(3*t-1, 1)")
  ...
  SECTION("(1000+((1-(1+sin(1/2+3*pi/3*t))*(2+t))*abs(t)^(2-1/2)))^(3/2)")
  {
    ...
    ER3 = 3;
    node =
      ruetsf.makeShared("(ER0+((ER1-(ER1+sin(ER1/ER2+ER3*pi/"
                        "ER3*t))*(ER2+t))*abs(t)^(ER2-ER1/ER2)))^(ER3/ER2)");
    y = { 52714.18945957, 32171.06013344, 31838.59857288, 31637.35056204,
          31624.96953355, 31622.77660168, 31588.17650985, 31631.21333073,
          30466.17238908, 31625.49824345, 10196.5206164 };
    REQUIRE(validate(node, t, y));
  ...
  SECTION("cos(3*t+atan2(t,1)-5*atan2(1/1+1-1,t/t*t)^(2*sqrt(abs(t))))")
  {
    ...
    ER2 = 5;
    ER3 = 2;
    node = ruetsf.makeShared("cos(ER0*t+atan2(t,ER1)-ER2*atan2(ER1/"
                             "ER1+ER1-ER1,t/t*t)^(ER3*sqrt(abs(t))))");
    y = { -0.45058035, -0.83062706, 0.94696144, -0.49875059,
          0.79075789,  0.28366219,  0.37464262, 0.0799824,
          -0.9787685,  -0.38051728, 0.99847693 };
    REQUIRE(validate(node, t, y));
