...
TEST_CASE("Survey Copy Test") {
    ...
    survey->scanner = std::make_shared<SingleScanner>(
        0.1, glm::dvec3(2.0, 3.0, 0.0), Rotation(0.0, 0.0, 0.0, 0.0, true),
        pulseFreqs, 4.0, "MyScanner", 80.5, 3.0, 0.9, 0.7, 0.8, 100, false, false, false, true, false
    );
    survey->scanner->setScannerHead(
        std::make_shared<ScannerHead>(glm::dvec3(0.4, 0.7, 0.1), 0.067)
    );
    survey->scanner->setBeamDeflector(
        std::make_shared<OscillatingMirrorBeamDeflector>(3.141592, 1400.5, 70.8, 1)
    );
    ...
    baseScene->primitives[0]->part->mPrimitives.push_back(baseScene->primitives[0]);
    baseScene->primitives[0]->part->onRayIntersectionMode = "TRANSMITTIVE";

    baseScene->primitives.push_back(
        new DetailedVoxel(glm::dvec3(0.0, 0.0, 0.5), 2.15, std::vector<int>({1, 2}), std::vector<double>({0.1, 0.2, 0.3}))
    );
    ...
    Rotation& copyMRA = copy->scanner->getScannerHead()->getMountRelativeAttitudeByReference();
    ...
    REQUIRE(copy->scanner->isCalcEchowidth() == survey->scanner->isCalcEchowidth());
    REQUIRE(copy->scanner->getFWFSettings().minEchoWidth != survey->scanner->getFWFSettings().minEchoWidth);
    REQUIRE(copy->scanner->getFWFSettings().apertureDiameter == survey->scanner->getFWFSettings().apertureDiameter);
    REQUIRE(copy->scanner->getScannerHead()->getRotatePerSecMax() == survey->scanner->getScannerHead()->getRotatePerSecMax());
    Rotation& baseMRA = survey->scanner->getScannerHead()->getMountRelativeAttitudeByReference();
    REQUIRE(copyMRA.getQ0() == baseMRA.getQ0());
    REQUIRE(copyMRA.getQ3() != baseMRA.getQ3());

    REQUIRE(copy->scanner->getBeamDeflector()->cfg_device_scanFreqMin_Hz == survey->scanner->getBeamDeflector()->cfg_device_scanFreqMin_Hz);
    REQUIRE(copy->scanner->getBeamDeflector()->cfg_device_scanFreqMax_Hz != survey->scanner->getBeamDeflector()->cfg_device_scanFreqMax_Hz);
    REQUIRE(copy->scanner->platform->cfg_device_relativeMountPosition.x != survey->scanner->platform->cfg_device_relativeMountPosition.x);
    REQUIRE(copy->scanner->platform->cfg_device_relativeMountPosition.y == survey->scanner->platform->cfg_device_relativeMountPosition.y);

    HelicopterPlatform* copyHp = static_cast<HelicopterPlatform*>(copy->scanner->platform.get());
    HelicopterPlatform* baseHp = static_cast<HelicopterPlatform*>(survey->scanner->platform.get());
    ...
    REQUIRE(copy->legs[0]->mPlatformSettings->onGround != survey->legs[0]->mPlatformSettings->onGround);
    REQUIRE(copy->legs[0]->mPlatformSettings->stopAndTurn == survey->legs[0]->mPlatformSettings->stopAndTurn);

    REQUIRE(copyScene->primitives[0]->getVertices()[0].pos.x != baseScene->primitives[0]->getVertices()[0].pos.x);
    REQUIRE(copyScene->primitives[0]->getVertices()[0].pos.y == baseScene->primitives[0]->getVertices()[0].pos.y);
    REQUIRE(copyScene->primitives[0]->part->onRayIntersectionArgument != baseScene->primitives[0]->part->onRayIntersectionArgument);
    REQUIRE(copyScene->primitives[1]->material->ks[0] == baseScene->primitives[1]->material->ks[0]);
    REQUIRE(copyScene->primitives[1]->material->ks[1] != baseScene->primitives[1]->material->ks[1]);
