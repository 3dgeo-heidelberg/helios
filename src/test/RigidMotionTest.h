...
RigidMotionTest::run()
{
  ...
  // Run tests
  if (!testPureRigidMotion())
    return false;
  if (!testHeliosRigidMotion())
    return false;
  return true;
}
...
// ******************* //
bool
RigidMotionTest::testPureRigidMotion()
{
  // R2 tests
  if (!testPureIdentityR2())
    return false;
  if (!testPureTranslationR2())
    return false;
  if (!testPureReflectionR2())
    return false;
  if (!testPureGlideReflectionR2())
    return false;
  if (!testPureRotationR2())
    return false;

  // R3 tests
  if (!testPureIdentityR3())
    return false;
  if (!testPureTranslationR3())
    return false;
  if (!testPureReflectionR3())
    return false;
  if (!testPureGlideReflectionR3())
    return false;
  if (!testPureRotationR3())
    return false;
  if (!testPureHelicalR3())
    return false;
  if (!testPureRotationalSymmetryR3())
    return false;

  return true;
}
bool
RigidMotionTest::testHeliosRigidMotion()
{
  ...
  // R3 tests
  if (!testHeliosIdentityR3(dmo))
    return false;
  if (!testHeliosTranslationR3(dmo))
    return false;
  if (!testHeliosReflectionR3(dmo))
    return false;
  if (!testHeliosGlideReflectionR3(dmo))
    return false;
  if (!testHeliosRotationR3(dmo))
    return false;
  if (!testHeliosHelicalR3(dmo))
    return false;
  if (!testHeliosRotationalSymmetryR3(dmo))
    return false;
...
// ******************** //
bool
RigidMotionTest::testPureIdentityR2()
{
...
bool
RigidMotionTest::testPureTranslationR2()
{
...
bool
RigidMotionTest::testPureReflectionR2()
{
  ...
  mat L = rme.computeFixedPoints(f, dim);
  if (L(0, 0) < 0.0)
    L = -L; // Solve sign ambiguity wrt expected solution
...
bool
RigidMotionTest::testPureGlideReflectionR2()
{
...
bool
RigidMotionTest::testPureIdentityR3()
{
...
bool
RigidMotionTest::testPureTranslationR3()
{
...
bool
RigidMotionTest::testPureReflectionR3()
{
  colvec ortho(std::vector<double>({ 1.0, 0.5, 0.3 }));
  RigidMotion f = rm3f.makeReflection(ortho);
  mat Y = rme.apply(f, R3X);
  mat EY("0.3358209 0.24626866 -0.82835821 1.41044776 -1.99253731; "
...
bool
RigidMotionTest::testPureGlideReflectionR3()
{
  colvec ortho(std::vector<double>({ 1.0, 0.5, 0.3 }));
  colvec shift(std::vector<double>({ -4.0 / 5.0, 1, 1 }));
  RigidMotion f = rm3f.makeGlideReflection(ortho, shift);
  mat Y = rme.apply(f, R3X);
  mat EY = ("-0.4641791   -0.55373134  -1.62835821   0.61044776  -2.79253731;"
...
bool
RigidMotionTest::testPureRotationR3()
{
  colvec axis(std::vector<double>({ 0.4, 0.2, 1.0 }));
  double theta = 4.5;
  RigidMotion f = rm3f.makeRotation(axis, theta);
  mat Y = rme.apply(f, R3X);
  mat EY("1.4704453    0.92147891  -3.31340313   5.70532734  -8.09725156;"
  ...
  Y = rme.apply(f, R3X);
  EY = mat("1.01575456   2.54922319  -6.11420095   9.6791787  -13.24415645;"
           "1.5          1.5         -4.5          7.5        -10.5;"
  ...
  Y = rme.apply(f, R3X);
  EY = mat("1.99328468   1.57169308  -5.13667083   8.70164858 -12.26662633;"
...
bool
RigidMotionTest::testPureHelicalR3()
{
  colvec axis(std::vector<double>({ 0.4, 0.2, 1.0 }));
  double theta = 4.5;
  double glide = 5.0;
  ...
  mat Y = rme.apply(f, R3X);
  mat EY("3.29618716  2.74722077 -1.48766127  7.5310692  -6.2715097;"
...
bool
RigidMotionTest::testPureRotationalSymmetryR3()
{
  colvec axis(std::vector<double>({ 1.0, 0.5, 0.3 }));
  double theta = 4.5;
  RigidMotion f = rm3f.makeRotationalSymmetry(axis, theta);
  mat Y = rme.apply(f, R3X);
  mat EY("2.23713056   1.83545467  -5.90803989   9.98062512 -14.05321035;"
  ...
  Y = rme.apply(f, R3X);
  EY = mat("3.47911047   3.07743457  -4.66605999  11.22260503 -12.81123044;"
...
// ********************** //
bool
RigidMotionTest::testHeliosIdentityR3(DynMovingObject const& dmo)
{
  // Compute
  ...
  // Delete primitives
  for (Primitive* primitive : dynObj.getPrimitives())
    delete primitive;
  ...
  for (size_t i = 0; i < m; ++i)
    if (std::fabs(dmeY[i] - rmeY[i]) > eps)
      return false;
  return true;
}
bool
RigidMotionTest::testHeliosTranslationR3(DynMovingObject const& dmo)
{
  // Compute
  arma::mat X = dmo.positionMatrixFromPrimitives();
  DynMovingObject dynObj = dmo;
...
bool
RigidMotionTest::testHeliosReflectionR3(DynMovingObject const& dmo)
{
  // Compute
  arma::mat X = dmo.positionMatrixFromPrimitives();
...
bool
RigidMotionTest::testHeliosGlideReflectionR3(DynMovingObject const& dmo)
{
  // Compute
  arma::mat X = dmo.positionMatrixFromPrimitives();
  arma::colvec O = dmo.getCentroid();
  ...
  DynMotion dm(
    rm3f.makeGlideReflection(arma::colvec("0;0;1"), arma::colvec("0.5;0.5;0")),
    true);
  arma::mat dmeY = dme.apply(dm, X, dynObj);
  arma::mat rmeY = rme.apply(rf, X);
...
bool
RigidMotionTest::testHeliosRotationR3(DynMovingObject const& dmo)
{
  // Compute
  arma::mat X = dmo.positionMatrixFromPrimitives();
  arma::colvec O = dmo.getCentroid();
...
bool
RigidMotionTest::testHeliosHelicalR3(DynMovingObject const& dmo)
{
  // Compute
  arma::mat X = dmo.positionMatrixFromPrimitives();
  arma::colvec O = dmo.getCentroid();
...
bool
RigidMotionTest::testHeliosRotationalSymmetryR3(DynMovingObject const& dmo)
{
  // Compute
  arma::mat X = dmo.positionMatrixFromPrimitives();
  arma::colvec O = dmo.getCentroid();
  ...
  RigidMotion rs = rme.compose(
    rm3f.makeTranslation(O),
    rme.compose(rm3f.makeRotationalSymmetryZ(0.34), rm3f.makeTranslation(-O)));
