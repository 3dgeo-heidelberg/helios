...
NoiseTest::run()
{
  ...
  // Noise tests
  if (uns1.next() != uns3.next())
    return false;
  if (uns2.next() == uns3.next())
    return false;
  if (uns3.next() != uns4.next())
    return false;
  if (uns1.next() != uns3.next())
    return false;
  if (uns1.next() == uns3.next())
    return false;
  if (nns4.next() == uns4.next())
    return false;
  if (nns4.next() == nns4.next())
    return false;
  if (uns2.next() == uns2.next())
    return false;
  ...
  if (uns1.next() != uns1m.next())
    return false;
  uns1m.next();
  if (uns1.next() == uns1m.next())
    return false;
  NormalNoiseSource<double> nns1c(nns1);
  if (nns1.next() != nns1c.next())
    return false;
  nns1c.next();
  if (nns1.next() == nns1c.next())
    return false;
