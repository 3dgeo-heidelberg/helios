...
public:
   ...
   */
  RayIntersectionTest()
    : BaseTest("Ray intersection test")
  {
  }
...
RayIntersectionTest::run()
{
  ...
  if (!ihr.canRayContinue())
    return false; // Ray must be able to continue
  ...
  if (ihr.canRayContinue())
    return true; // Ray must be able to continue
  ...
  if (ihr.canRayContinue())
    return false; // Ray must NOT be able to continue
