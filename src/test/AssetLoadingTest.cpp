...
TEST_CASE("Asset Loading Tests")
{
  ...
  SECTION("Test Scanner Loading")
  {
    ...
    REQUIRE(std::dynamic_pointer_cast<OscillatingMirrorBeamDeflector>(
    ...
    REQUIRE(std::dynamic_pointer_cast<PolygonMirrorBeamDeflector>(
      scanner->getBeamDeflector()));
    pfs = scanner->getSupportedPulseFreqs_Hz();
    REQUIRE(pfs.size() == 5);
    pfsit = pfs.begin();
    ...
    REQUIRE(std::fabs(scanner->getBeamDeflector()->cfg_device_scanAngleMax_rad -
                      0.7853982) <= eps);
    REQUIRE(std::fabs(std::dynamic_pointer_cast<PolygonMirrorBeamDeflector>(
                        scanner->getBeamDeflector())
    ...
    REQUIRE(std::dynamic_pointer_cast<ConicBeamDeflector>(
      scanner->getBeamDeflector()));
    pfs = scanner->getSupportedPulseFreqs_Hz();
    REQUIRE(pfs.size() == 4);
    pfsit = pfs.begin();
    ...
    REQUIRE_FALSE(std::dynamic_pointer_cast<RisleyBeamDeflector>(
    ...
    REQUIRE(std::fabs(epdiff[2]) <= eps);
    epdiff =
      scanner->getHeadRelativeEmitterPosition(1) - glm::dvec3(0, 0, -0.1);
    REQUIRE(std::fabs(epdiff[0]) <= eps);
    REQUIRE(std::fabs(epdiff[1]) <= eps);
    REQUIRE(std::fabs(epdiff[2]) <= eps);
  ...
  SECTION("Test Platform Loading")
  {
    ...
    REQUIRE(nns);
    REQUIRE_FALSE(std::dynamic_pointer_cast<UniformNoiseSource<double>>(
      platf->positionXNoiseSource));
