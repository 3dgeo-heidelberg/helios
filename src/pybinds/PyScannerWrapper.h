...
public:
  ...
  {
    scanner.buildScanningPulseProcess(parallelizationStrategy, dropper, pool);
  }
  ...
  std::string toString() { return scanner.toString(); }
  ...
  inline int getNumRays() const { return scanner.getNumRays(); }
  inline int getNumRays(size_t const idx) const
  {
    return scanner.getNumRays(idx);
  }
  inline void setNumRays(int const numRays) { scanner.setNumRays(numRays); }
  ...
  inline bool lastPulseWasHit() const { return scanner.lastPulseWasHit(); }
  ...
  inline double getAveragePower() const { return scanner.getAveragePower(0); }
  ...
  inline double getBeamQuality() const { return scanner.getBeamQuality(0); }
  ...
  inline double getEfficiency() const { return scanner.getEfficiency(0); }
  ...
  inline double getWavelength() const { return scanner.getWavelength(0); }
  ...
  double getBt2(size_t const idx) const { return scanner.getBt2(idx); }
  inline double getBt2() const { return scanner.getBt2(0); }
  void setBt2(double const bt2, size_t const idx) { scanner.setBt2(bt2, idx); }
  inline void setBt2(double const bt2) { scanner.setBt2(bt2, 0); }
  double getDr2(size_t const idx) const { return scanner.getDr2(idx); }
  inline double getDr2() const { return scanner.getDr2(0); }
  void setDr2(double const dr2, size_t const idx) { scanner.setDr2(dr2, idx); }
  inline void setDr2(double const dr2) { scanner.setDr2(dr2, 0); }
  inline bool isActive() const { return scanner.isActive(); }
  inline void setActive(bool const active) { scanner.setActive(active); }
  ...
  inline bool isCalcEchowidth() const { return scanner.isCalcEchowidth(); }
  inline void setCalcEchowidth(bool const calcEchowidth)
  {
    scanner.setCalcEchowidth(calcEchowidth);
  }
  inline bool isFullWaveNoise() const { return scanner.isFullWaveNoise(); }
  ...
  inline std::string getScannerId() const { return scanner.getScannerId(); }
  inline void setScannerId(std::string const& id) { scanner.setScannerId(id); }
  std::string getDeviceId(size_t const idx) const
  {
    return scanner.getDeviceId(idx);
  }
  inline std::string getDeviceId() const { return scanner.getDeviceId(0); }
  ...
  size_t getNumDevices() { return scanner.getNumDevices(); }
  ...
  {
    return new PythonDVec3(scanner.getHeadRelativeEmitterPositionByRef(idx));
  }
  ...
  {
    if (scanner.intersectionHandlingNoiseSource == nullptr)
      return nullptr;
    return new PyNoiseSourceWrapper(*scanner.intersectionHandlingNoiseSource);
  }
  PyRandomnessGeneratorWrapper* getRandGen1()
  {
    if (scanner.randGen1 == nullptr)
      return nullptr;
    return new PyRandomnessGeneratorWrapper(*scanner.randGen1);
  }
  PyRandomnessGeneratorWrapper* getRandGen2()
  {
    if (scanner.randGen2 == nullptr)
      return nullptr;
