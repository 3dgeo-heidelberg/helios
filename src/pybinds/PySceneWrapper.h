...
public:
  ...
  PyAABBWrapper* getAABB() { return new PyAABBWrapper(scene.getAABB().get()); }
  PythonDVec3* getGroundPointAt(double x, double y, double z)
  {
    glm::dvec3 gp = glm::dvec3(x, y, z);
    return new PythonDVec3(gp);
  }
  PyRaySceneIntersectionWrapper* getIntersection(double ox,
                                                 double oy,
                                                 double oz, // Origin
                                                 double dx,
                                                 double dy,
  ...
  PyAABBWrapper* getBBox() { return new PyAABBWrapper(scene.getBBox().get()); }
