...
KDTREE_FACTORY_EXTRACT_GSTYPE(MultiThreadKDTreeFactory const* mtkdtf)
{
  ...
  if (mtkdtf->getGS() == nullptr) { // No geometric strategy : 0
    gsType = 0;
  } else if (std::dynamic_pointer_cast<FastSAHKDTreeGeometricStrategy>(
               mtkdtf->getGS()) !=
             nullptr) { // Fast SAH KDTree geometric strategy : 4
    gsType = 4;
  } else if (std::dynamic_pointer_cast<AxisSAHKDTreeGeometricStrategy>(
               mtkdtf->getGS()) !=
             nullptr) { // Axis SAH KDTree geometric strategy : 3
    gsType = 3;
  } else if (std::dynamic_pointer_cast<SAHKDTreeGeometricStrategy>(
               mtkdtf->getGS()) !=
