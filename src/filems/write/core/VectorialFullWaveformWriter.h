...
public:
    ...
    shared_ptr<SyncFileWriter<std::vector<FullWaveform> const &>> makeWriter(
        ...
        if(isZipOutput()){
            return make_shared<ZipVectorialSyncFileFullWaveformWriter>(
                path
            );
        }
        else return make_shared<SimpleVectorialSyncFileFullWaveformWriter>(
            path
        );
    }
};
