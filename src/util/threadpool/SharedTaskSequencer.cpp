...
// ****************************** //
void
SharedTaskSequencer::start(std::shared_ptr<SharedSubTask> subTask)
{
...
void
SharedTaskSequencer::startThread(std::shared_ptr<SharedSubTask> subTask)
{
...
void
SharedTaskSequencer::onSharedSubTaskCompletion(size_t const key)
{
  boost::unique_lock<boost::mutex> uniqueLock(mtx);
  if (maxThreads > 0)
    ++availableThreads;
