...
// ***  STATIC METHODS  *** //
// ************************ //
template <typename T> T SurfaceInspector::maths::Statistics::mean(std::vector<T> const &vals){
    T mean = 0;
    for(T const &val : vals) mean += val;
    return mean / ((T)vals.size());
}
template <typename T> T SurfaceInspector::maths::Statistics::variance(std::vector<T> const &vals){
    T mean = Statistics::mean(vals);
    return Statistics::variance(vals, mean);
}
template <typename T>
T SurfaceInspector::maths::Statistics::variance(std::vector<T> const &vals, T const mean){
    T variance = 0;
...
template <typename T>
T SurfaceInspector::maths::Statistics::stdev(std::vector<T> const &vals){
    T variance = SurfaceInspector::maths::Statistics::variance(vals);
    return SurfaceInspector::maths::Statistics::stdev(variance);
}
template <typename T>
T SurfaceInspector::maths::Statistics::stdev(std::vector<T> const &vals, T const mean){
    T variance = SurfaceInspector::maths::Statistics::variance(vals, mean);
    return SurfaceInspector::maths::Statistics::stdev(variance);
}
template <typename T>
T SurfaceInspector::maths::Statistics::covariance(
...
template <typename T>
arma::Mat<T> SurfaceInspector::maths::Statistics::covarianceMatrix(
    std::vector<std::vector<T>> dataset,
    bool besselCorrection
){
    // Compute means
    std::vector<T> means;
    for(std::vector<T> &vals : dataset) means.push_back(SurfaceInspector::maths::Statistics::mean(vals));
...
template <typename T> std::vector<T> SurfaceInspector::maths::Statistics::quantiles(
    arma::Col<T> data,
    ...
    std::vector<T> Q;
    for(std::size_t i = 0 ; i < nQuantiles ; i++) Q.push_back(_Q[i]);
...
template <typename T> std::vector<T> SurfaceInspector::maths::Statistics::quantiles(
    ...
    arma::Col<T> vec(n);
    for(std::size_t i = 0 ; i < n ; i++) vec[i] = data[i];
    return quantiles(vec, nQuantiles);
}

template <typename T> std::size_t SurfaceInspector::maths::Statistics::quantileFiltering(
    std::vector<T> &values,
    std::size_t nQuantiles,
    double filterFactor
){
...
template <typename T> bool SurfaceInspector::maths::Statistics::findTrustableMin(
    ...
    SurfaceInspector::util::draggers::MinDragger<T> drg = SurfaceInspector::util::draggers::MinDragger<T>(std::vector<T>(S)); // To drag min1, min2, ...
    return _findTrustable(S, m, tau, drg, g, discardsCount);
}

template <typename T> bool SurfaceInspector::maths::Statistics::findTrustableMax(
    std::vector<T> const & S,
    std::size_t const m,
    T const tau,
    T &g,
    ...
    // Compute general case
    SurfaceInspector::util::draggers::MaxDragger<T> drg = SurfaceInspector::util::draggers::MaxDragger<T>(std::vector<T>(S)); // To drag max1, max2, ...
    return _findTrustable(S, m, tau, drg, g, discardsCount);
}

// ***  INNER METHODS  *** //
// *********************** //
template <typename T> bool SurfaceInspector::maths::Statistics::_findTrustable(
    std::vector<T> const & S,
    std::size_t const m,
    T const tau,
    ...
    for(std::size_t i = 0 ; i < m ; ++i) Z[i] = drg.next();
    D = SurfaceInspector::maths::Vector<T>::diff(Z); // Delta as discrete differences of Z
    ...
    while(!check) { // While !check ~ While checking not finished
        ...
        for (std::size_t i = m_2; i >= 0; --i) { // The delta check itself
            if(std::fabs(D[i]) > tau){ // Handle too big difference case
                ...
                for(std::size_t j=i; j<m_1; ++j)Z[j-i] = Z[j+1]; // Move to the left
                for(std::size_t j=m_1-i; j<m; ++j)Z[j]=drg.next();//Add to the right
                D = SurfaceInspector::maths::Vector<T>::diff(Z); // Update discrete differences of Z
