...
public:
    ...
    {
        this->rg.computeUniformRealDistribution(
            uniformNoiseMin,
            uniformNoiseMax
        );
        this->build();
...
UniformNoiseSource<RealType>::configureUniformNoise(
    RealType min,
    RealType max
){
...
// ************************** //
template<typename RealType>
std::ostream& operator<<(std::ostream &out, UniformNoiseSource<RealType> &ns){
