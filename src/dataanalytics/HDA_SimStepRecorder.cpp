...
void
HDA_SimStepRecorder::delayedRecord()
{
  recordStochastic();
}
...
// ************************** //
bool
HDA_SimStepRecorder::isAnyBufferOpen()
{
  bool anyOpen = false;
...
void
HDA_SimStepRecorder::openBuffers()
{
  ...
  beamYaw =
    std::make_shared<HDA_RecordBuffer<double>>(craftOutputPath("beam_yaw.csv"));
...
void
HDA_SimStepRecorder::closeBuffers()
{
...
// ********************************* //
void
HDA_SimStepRecorder::recordPlatform()
{
  // Obtain platform
...
void
HDA_SimStepRecorder::recordPlatformMount()
{
  // Obtain platform
  Platform& p = *(sp->getScanner()->platform);
...
void
HDA_SimStepRecorder::recordScanner()
{
  // Obtain scanner
...
void
HDA_SimStepRecorder::recordScannerHead()
{
...
void
HDA_SimStepRecorder::recordDeflector()
{
...
void
HDA_SimStepRecorder::recordBeam()
{
  // Obtain scanner
  Platform& p = *(sp->getScanner()->platform);
  Scanner& s = *(sp->getScanner());
  // Record beam origin
  glm::dvec3 bo =
    p.getAbsoluteMountPosition() + s.getHeadRelativeEmitterPosition();
...
void
HDA_SimStepRecorder::recordStochastic()
{
  ...
  // Prepare fake pulse
  SimulatedPulse fakePulse(glm::dvec3(0, 0, 0), Rotation(), 0.0, 0, 0, 0);
