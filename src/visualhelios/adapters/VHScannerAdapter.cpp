...
void
VHScannerAdapter::startLeg(unsigned int const legIndex, bool const manual)
{
    ...
    if (nextLegIndex < survey.legs.size()) {
      ...
      if (nextLegIndex + 1 < survey.legs.size()) {
        platform.setAfterDestination(
          survey.legs.at(nextLegIndex + 1)->mPlatformSettings->getPosition());
      } else {
        platform.setAfterDestination(nextLeg.mPlatformSettings->getPosition());
      }
    }
...
void
VHScannerAdapter::stopAndTurn(unsigned int legIndex, shared_ptr<Leg> leg)
{
  // Check that the platform is a HelicopterPlatform, otherwise return
  HelicopterPlatform* hp =
    dynamic_cast<HelicopterPlatform*>(scanner.platform.get());
  if (hp == nullptr)
    return;

  if (legIndex == survey.legs.size() - 1)
    return;
