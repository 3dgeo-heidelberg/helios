  ...
  : dynScene(dynScene)
{
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (dynObj->getType() == ScenePart::ObjectType::DYN_MOVING_OBJECT) {
      static_pointer_cast<DynMovingObject>(dynObj)->unregisterObserverGrove();
    }
  }
}
...
template<typename ST, typename DT>
bool
VHDynSceneAdapter<ST, DT>::doStep()
{
  // Compute steps
  ...
  for (size_t i = 0; i < m; ++i) {
    if (isDynObjectUpdated(i))
      getAdaptedDynObj(i)->doStep(false, true);
  }
