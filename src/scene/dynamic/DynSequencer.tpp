...
template<typename T>
void
DynSequencer<T>::add(std::shared_ptr<DynSequence<T>> dynseq)
{
  // Insert on map
...
template<typename T>
std::shared_ptr<DynSequence<T>>
DynSequencer<T>::get(std::string const& id)
{
  if (!has(id))
    return nullptr;
  return dynseqs[id];
