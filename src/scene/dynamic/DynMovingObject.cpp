...
DynMovingObject::_next(deque<shared_ptr<DynMotion>>& deck)
{
...
DynMovingObject::doSimStep()
{
  ...
  modified |= applyDynMotionQueue(
    ...
    [=]() -> shared_ptr<DynMotion> { return this->nextPositionMotion(); });
  ...
  modified |= applyDynMotionQueue(
    ...
    [=]() -> shared_ptr<DynMotion> { return this->nextNormalMotion(); });
  ...
  if (queueHasNext()) {
    ...
    while (queueHasNext()) {
      if (dm == nullptr)
        ...
        dm = make_shared<DynMotion>(dme.compose(*queueNext(), *dm, *this));
      }
    }
