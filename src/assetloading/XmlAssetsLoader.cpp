  ...
  std::shared_ptr<FWFSettings> fwfSettings)
{
  ...
  int scanner_maxnor; // max number of returns per pulse; defined in scanner node
  scanner_maxnor = XmlUtils::getAttributeCast<int>(scannerNode, "maxNOR", 0);
  while (chan != nullptr) { // Update i-th device with i-th channel
    ...
    scanner->setMaxNOR(XmlUtils::getAttributeCast<int>(chan, "maxNOR", scanner_maxnor), idx);
