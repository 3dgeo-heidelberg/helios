...
SurveyPlayback::SurveyPlayback(
    shared_ptr<Survey> survey, const std::string outputPath,
    int const parallelizationStrategy,
    std::shared_ptr<PulseThreadPoolInterface> pulseThreadPoolInterface,
    int const chunkSize, std::string fixedGpsTimeStart, bool lasOutput,
    bool las10, bool zipOutput, bool exportToFile)
    : Simulation(parallelizationStrategy, pulseThreadPoolInterface, chunkSize,
                 fixedGpsTimeStart),
      lasOutput(lasOutput), las10(las10), zipOutput(zipOutput),
  ...
  ostringstream oss;
  oss << outputPath << "Survey Playback/" << mSurvey->name << "/" << mDateString
      << "/";
...
void SurveyPlayback::estimateTime(int legCurrentProgress, bool onGround,
  ...
  if (legCurrentProgress >
    ...
    legRemainingTime_ms =
        (long)((100 - legProgress) / legProgress * legElapsedTime_ms);
    ...
    remainingTime_ms = (long)((100 - progress) / progress * elapsedTime_ms);
    ...
    oss << "Survey " << progress << "%\tElapsed "
        << milliToString(elapsedTime_ms) << " Remaining "
        << milliToString(remainingTime_ms) << endl;
    oss << "Leg" << (mCurrentLegIndex + 1) << "/" << (numEffectiveLegs) << " "
        << legProgress << "%\tElapsed " << milliToString(legElapsedTime_ms)
        << " Remaining " << milliToString(legRemainingTime_ms);
...
void SurveyPlayback::trackProgress() {
  if (!getScanner()->platform->canMove()) {
    double legElapsedAngle =
        std::fabs(getScanner()->scannerHead->getRotateStart() -
                  getScanner()->scannerHead->getRotateCurrent());
    int legProgress = (int)(legElapsedAngle * 100 /
                            getScanner()->scannerHead->getRotateRange());
    ...
    int legProgress =
        (int)(legElapsedLength * 100 / getCurrentLeg()->getLength());
...
int SurveyPlayback::getCurrentLegIndex() { return mCurrentLegIndex; }
  ...
  if (leg->mPlatformSettings != nullptr) {
    ...
    if (nextLegIndex < mSurvey->legs.size()) {
      ...
      if (nextLegIndex + 1 < mSurvey->legs.size()) {
        platform->setAfterDestination(mSurvey->legs.at(nextLegIndex + 1)
...
void SurveyPlayback::prepareOutput() {
  ...
  shared_ptr<FullWaveformPulseDetector> fwf_detector =
      dynamic_pointer_cast<FullWaveformPulseDetector>(getScanner()->detector);
  // Fullwave prefix
  stringstream ss;
  ss << getLegOutputPrefix();
  if (zipOutput) {
    ...
    ss << "_fullwave.txt";
  fwf_detector->setOutputFilePath(mOutputFilePathString +
                                      getCurrentOutputPath(),
                                  ss.str(), getScanner()->isWriteWaveform());

  // Trajectory writer
  if (mSurvey->scanner->trajectoryTimeInterval != 0.0) {
    std::string trajectoryOutputPath =
        mOutputFilePathString + getTrajectoryOutputPath();
...
void SurveyPlayback::clearPointcloudFile() {
  ...
  string outputPath =
      this->outputPath + mOutputFilePathString + getCurrentOutputPath();
  ostringstream s;
  s << "outputPath=" << outputPath << endl;
  logging::INFO(s.str());
