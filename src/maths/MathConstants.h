#pragma once

/**
 * \f[
 *  \pi
 * \f]
 */
constexpr double PI = 3.14159265358979323846;

/**
 * @brief Euler's number
 */
constexpr double EULER_NUMBER = 2.71828182845904523536;

/**
 * \f[
 *  \frac{\pi}{2}
 * \f]
 */
constexpr double PI_HALF = PI / 2.0;

/**
 * \f[
 *  \frac{\pi}{4}
 * \f]
 */
constexpr double PI_QUARTER = PI / 4.0;
/**
 * \frac[
 *  \frac{\pi}{8}
 * \f]
 */
constexpr double PI_EIGHTH = PI / 8.0;

/**
 * \f[
 *  \frac{3{\pi}}{2}
 * \f]
 */
constexpr double PI_3_HALF = PI_HALF * 3.0;

/**
 * \f[
 *  2{\pi}
 * \f]
 */
constexpr double PI_2 = PI * 2.0;

/**
 * \f[
 *  4{\pi}
 * \f]
 */
constexpr double PI_4 = PI * 4.0;
/**
 * \f[
 *  \pi^2
 * \f]
 */
constexpr double PI_SQUARED = PI * PI;

/**
 * \f[
 *  2 \pi^2
 * \f]
 */
constexpr double PI_SQUARED_2 = 2 * PI * PI;

/**
 * \f[
 *  \frac{180}{\pi}
 * \f]
 */
constexpr double _180_OVER_PI = 180.0 / PI;

/**
 * \f[
 *  \frac{\pi}{180}
 * \f]
 */
constexpr double PI_OVER_180 = PI / 180.0;

/**
 * @brief Constant representing a very close from above to -1 number
 */
constexpr double ALMOST_MINUS_1 = -0.9999999999;
/**
 * @brief Constant representing a very close from below to 1 number
 */
constexpr double ALMOST_PLUS_1 = 0.9999999999;

/**
 * \f[
 *  \sqrt{2}
 * \f]
 */

constexpr double SQRT2 = 1.4142135623730951;

/**
 * @brief Speed of light in meters per second
 */
constexpr double SPEEDofLIGHT_mPerSec = 299792458;

/**
 * @brief Speed of light in meters per nanosecond
 */
constexpr double SPEEDofLIGHT_mPerNanosec = 0.299792458;

/**
 * @brief Speed of light in meters per picosecond
 */
constexpr double SPEEDofLIGHT_mPerPicosec = 0.000299792458;
