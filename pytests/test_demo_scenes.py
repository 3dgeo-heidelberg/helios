...
def eval_detailedVoxels_uls(dirname):
    assert (dirname / 'leg000_points.las').exists()
    assert abs((dirname / 'leg000_points.las').stat().st_size - 584_687) < MAX_DIFFERENCE_BYTES
...
def eval_quadcopter(dirname):
    ...
    assert (dirname / 'leg000_trajectory.txt').exists()
    assert abs(
        (dirname / 'leg000_points.laz').stat().st_size - 1_974_904) < MAX_DIFFERENCE_BYTES  # 1_974_904 vs. Win: 1_973_177
    #assert abs(
    #    (dirname / 'leg002_points.laz').stat().st_size - 2_154_149) < MAX_DIFFERENCE_BYTES  # 2_154_149 vs. Win: 2_150_438
    #assert abs(
    #    (dirname / 'leg004_points.laz').stat().st_size - 3_819_553) < MAX_DIFFERENCE_BYTES  # 3_819_553 vs. Win: 3_813_762
