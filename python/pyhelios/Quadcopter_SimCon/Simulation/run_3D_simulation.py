...
from Quadcopter_SimCon.Simulation.ctrl import Control
...
from Quadcopter_SimCon.Simulation import utils
...
def main():
    print('enter height above ground (m)')
    height=float(input())
    print('select if yaw follows trajectory (enter "1") or is constantly zero (enter "2")')
    yaw_mode = int(input())+2
    if yaw_mode not in [3,4]:
        yaw_mode=3
    print('simulate quadrocopter trajectory. HELIOS simulation will start afterwards.')
    ...
    xyztrpy[:,3]=t_all
    xyztrpy[:,4:7]=euler_all/np.pi*180
    
    np.savetxt("./data/trajectories/xyztrpyquadro_click.trj",xyztrpy,delimiter=",")
    print('saved trajectory at "./data/trajectories/xyztrpyquadro_click.trj"')
