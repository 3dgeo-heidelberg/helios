...
wfs = OrderedDict()
with open(
    r"..\output\Survey Playback\als_hd_demo\points\leg000_fullwave.txt", "r"
) as inf:
...
pts = np.loadtxt(
    r"..\output\Survey Playback\als_hd_demo\points\leg000_points.xyz", delimiter=" "
)
...
las.z = xyz[:, 2]
las.intensity = pts[:, 3]
las.wavepacket_index = np.ones(
    (xyz.shape[0])
)  # 1 refers to the value 100 (99+1) in the VLR below
las.wavepacket_offset = 60 + np.array([wfs_lut_rev[idx] for idx in pts[:, 7]]) * (
    max_len
)
las.wavepacket_size = [max_len] * xyz.shape[
    0
]  # always write a wavepacket of the longest size
las.return_point_wave_location = [0] * xyz.shape[
    0
]  # adapt this if you need the information on which position this point
...
wf_data = b"".join(
    [wf_data, struct.pack("<Q", max_len * len(wfs))]
...
with open("test.las", "r+b") as f:
    f.seek(6)
    f.write(
        struct.pack("<H", int("0000000000000100", 2))
    )  # set "external waveform" bit to 1 in las file (not possible with laspy)
