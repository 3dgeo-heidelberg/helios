def read_obj(path, logging=False):
    ...
    for line in lines:
        ...
        if vals[0] == "f" and len(vals) > 4:
            count += 1
            max_vert_num = (
                len(vals) - 1 if len(vals) - 1 > max_vert_num else max_vert_num
            )
    ...
    if logging:
        print(
            "Total number of faces in OBJ with more than 3 vertices: {}".format(count)
        )
        print(
            "Total number of faces in OBJ with 4 vertices: {}".format(count_four_verts)
        )
        print("Maximum number of vertices per face: {}".format(max_vert_num))

    if max_vert_num > 4:
        print(
            "[HELIOS.PY WARNING] {} contains at least one face with more than 4 vertices. \
        These faces will not be visualised and are not considered by HELIOS++".format(
                path
            )
        )
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".obj", delete=False) as f:
            ...
            for line in lines:
                ...
                if linesplit[0] == "f" and len(linesplit) == 5:
                    ...
                    f.write(
                        "f {} {} {}".format(linesplit[1], linesplit[3], linesplit[4])
                    )
