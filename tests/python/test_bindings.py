...
def test_primitive_properties():
    ...
    assert (
        isinstance(triangle.centroid, tuple) and len(triangle.centroid) == 3
    ), "centroid should be a 3-tuple"
...
def test_primitive_ray_intersection():
    ...
    intersections = triangle.ray_intersection(ray_origin, ray_dir)
    assert isinstance(
        intersections, _helios.DoubleVector
    ), "ray_intersection should return a DoubleVector"
