  ...
  RandomnessGenerator const& rg)
{
  ...
  if (rg.urdGen == nullptr)
    this->urdGen = nullptr;
  else
    this->urdGen =
      std::unique_ptr<boost::mt19937>(new boost::mt19937(*rg.urdGen));
  if (rg.urd == nullptr)
    this->urdGen = nullptr;
  else
    this->urd =
  ...
  if (rg.ndGen == nullptr)
    ...
    this->ndGen =
      std::unique_ptr<boost::mt19937>(new boost::mt19937(*rg.ndGen));
  if (rg.nd == nullptr)
    this->nd = nullptr;
  else
    this->nd = std::unique_ptr<boost::normal_distribution<RealType>>(
      new boost::normal_distribution<RealType>(*rg.nd));
}

template<typename RealType>
RandomnessGenerator<RealType>::RandomnessGenerator(
  RandomnessGenerator&& rg) noexcept
{
  swap(*this, rg);
}
...
template<typename RealType>
void
RandomnessGenerator<RealType>::swap(RandomnessGenerator<RealType>& rgA,
...
template<typename RealType>
void
RandomnessGenerator<RealType>::computeUniformRealDistribution(
  RealType lowerBound,
  RealType upperBound)
{
  if (mode == "AUTO_SEED") {
    ...
    urd = std::unique_ptr<boost::random::uniform_real_distribution<RealType>>(
      new boost::random::uniform_real_distribution<RealType>(lowerBound,
                                                             upperBound));
  } else if (mode == "FIXED_SEED_DOUBLE") {
    urdGen =
      std::unique_ptr<boost::mt19937>(new boost::mt19937(getDoubleSeed()));
    urd = std::unique_ptr<boost::random::uniform_real_distribution<RealType>>(
      new boost::random::uniform_real_distribution<RealType>(lowerBound,
                                                             upperBound));
  } else if (mode == "FIXED_SEED_LONG") {
    urdGen = std::unique_ptr<boost::mt19937>(new boost::mt19937(getLongSeed()));
    urd = std::unique_ptr<boost::random::uniform_real_distribution<RealType>>(
      new boost::random::uniform_real_distribution<RealType>(lowerBound,
                                                             upperBound));
  }
...
template<typename RealType>
RealType
RandomnessGenerator<RealType>::uniformRealDistributionNext()
{
  if (urd == nullptr)
    computeUniformRealDistribution(0.0, 1.0);
  return (*urd)(*urdGen);
}

template<typename RealType>
void
RandomnessGenerator<RealType>::computeNormalDistribution(RealType mean,
                                                         RealType stdev)
{
  if (mode == "AUTO_SEED") {
    boost::random_device rd;
    ...
    nd = std::unique_ptr<boost::normal_distribution<RealType>>(
      new boost::normal_distribution<RealType>(mean, stdev));
  } else if (mode == "FIXED_SEED_DOUBLE") {
    ndGen =
      std::unique_ptr<boost::mt19937>(new boost::mt19937(getDoubleSeed()));
    nd = std::unique_ptr<boost::normal_distribution<RealType>>(
    ...
    ndGen = std::unique_ptr<boost::mt19937>(new boost::mt19937(getLongSeed()));
    nd = std::unique_ptr<boost::normal_distribution<RealType>>(
      new boost::normal_distribution<RealType>(mean, stdev));
  }
}
...
template<typename RealType>
RealType
RandomnessGenerator<RealType>::normalDistributionNext()
{
  if (nd == nullptr)
    computeNormalDistribution(0.0, 1.0);
  return (*nd)(*ndGen);
}
