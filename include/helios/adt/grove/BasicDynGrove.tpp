...
void
BasicDynGrove<Tree, Subject>::update(Subject& s)
{
...
template<typename Tree, typename Subject>
void
BasicDynGrove<Tree, Subject>::addSubject(
  ...
  std::shared_ptr<Tree> tree)
{
  ...
  for (; lastSubjectId < id; ++lastSubjectId)
    subjects.push_back(nullptr);

  // Add subject to the grove
  trees.push_back(tree);
  subjects.push_back(subject);
  if (subject != nullptr)
    subject->setGroveSubjectId(id);
}

template<typename Tree, typename Subject>
void
BasicDynGrove<Tree, Subject>::removeSubject(
  BasicDynGroveSubject<Tree, Subject>* subject)
{
