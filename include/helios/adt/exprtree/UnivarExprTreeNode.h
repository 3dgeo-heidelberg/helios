...
#include <helios/adt/exprtree/IExprTreeNode.h>
...
public:
   ...
   */
  UnivarExprTreeNode(UnivarExprTreeNode* left = nullptr,
                     UnivarExprTreeNode* right = nullptr)
    : left(left)
    , right(right)
  {
  }
   ...
   *  \f$x \odot y\f$
   */
  inline NumericType doOperation(NumericType const x, NumericType const y) const
  {
    switch (op) {
      case OP_ADD:
        return x + y;
   ...
   */
  inline NumericType doFunction(NumericType const x) const
  {
    switch (fun) {
      case f_exp:
        return std::exp(x);
