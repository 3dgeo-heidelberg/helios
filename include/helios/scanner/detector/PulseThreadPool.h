...
#include <helios/util/threadpool/ResThreadPool.h>
#if DATA_ANALYTICS >= 2
...
class PulseThreadPool
  : public ResThreadPool<std::vector<std::vector<double>>&,
...
public:
   ...
   */
  explicit PulseThreadPool(std::size_t const _pool_size,
                           double const deviceAccuracy,
                           bool const dynamic)
    : ResThreadPool<std::vector<std::vector<double>>&,
                    RandomnessGenerator<double>&,
                    RandomnessGenerator<double>&,
                    NoiseSource<double>&
#if DATA_ANALYTICS >= 2
