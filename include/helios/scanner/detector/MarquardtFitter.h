...
class MarquardtFitter
{
  ...
  // ******************** //
  /*
   * for solving non-linear least squares fit of f(x:A) = z with sets of x,z
   * data points.
   */
  /**
...
public:
   ...
   * @param params Array that will be used to calculate derivatives, note params
   *will be modified.
   ...
   **/
  double calculateDerivative(int const k,
                             double const x,
   ...
   *  std::vector<double>&)
   */
  void calculateDerivativeFast(double const x,
                               double const c,
                               double const d,
