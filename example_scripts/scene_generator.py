...
metafile = open(f'{name_scene}_{number_segments}SP_{str(radius_scan_pos_circle).replace(".", "dot")}rad_surveySceneCombi.xyz', 'w')
...
for segment in range(0, number_segments):
	...
	circle_scan_pos_list_x.append(radius_scan_pos_circle * math.cos(math.radians(current_angle)))
	circle_scan_pos_list_y.append(radius_scan_pos_circle * math.sin(math.radians(current_angle)))
...
for i in range(0, number_segments):
	...
	distributed_scan_positions.append(f"""	
		...
		""")
	metafile.write(f"{i};{circle_scan_pos_list_x[i]};{circle_scan_pos_list_y[i]};0.0;ScanPos{i}\n")
...
coord_list_x = []
frange(coord_list_x, -10., 10., .5)

coord_list_y = []
frange(coord_list_y, -10., 10., .5)
